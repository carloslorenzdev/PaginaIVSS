...
@section('content')
    <x-section class="space-y-4 p-4 sm:space-y-6 sm:p-6">
        <x-titulo titulo="Nueva Categoría" />
        <x-breadcrumb>
        ...
        <x-card class="p-5 max-w-3xl mx-auto">
            <x-card.title class="mb-5 border-b border-gray-200 dark:border-neutral-700 pb-3">
                <i class="bx bx-plus-circle"></i> Crear Nueva Categoría
            </x-card.title>

            <form action="{{ route('admin.categorias.guardar') }}" method="POST" class="space-y-5">
                @csrf
                
                <div>
                    <x-input.label for="nombre" value="Nombre de la Categoría *" />
                    <x-input type="text" id="nombre" name="nombre" value="{{ old('nombre') }}" required maxlength="255" error />
                </div>
                
                <div>
                    <x-input.label for="descripcion" value="Descripción (Opcional)" />
                    <textarea name="descripcion" id="descripcion" rows="3"
                        class="py-3 px-4 block w-full border-gray-200 rounded-lg text-sm focus:border-red-500 focus:ring-red-500 disabled:opacity-50 disabled:pointer-events-none dark:bg-neutral-900 dark:border-neutral-700 dark:text-neutral-400 dark:placeholder-neutral-500 dark:focus:ring-neutral-600">{{ old('descripcion') }}</textarea>
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <x-input type="number" id="orden" name="orden" value="{{ old('orden', 0) }}" min="0" error />
                    </div>
                    <div class="flex items-center pt-8">
                        <label class="flex items-center cursor-pointer relative">
                            <input type="checkbox" name="activa" value="1" {{ old('activa', true) ? 'checked' : '' }} class="peer sr-only">
