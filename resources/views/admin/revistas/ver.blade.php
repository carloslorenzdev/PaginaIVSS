...
@section('titulo', 'Editar Revista Digital')

@section('content')
    <x-section class="space-y-4 p-4 sm:space-y-6 sm:p-6">
        <x-titulo titulo="Editar Revista" />
        <x-breadcrumb>
        ...
        <x-card class="p-5 max-w-3xl mx-auto">
            <x-card.title class="mb-4 border-b pb-2"><i class="bx bx-edit"></i> Detalles de la Revista</x-card.title>

            <form action="{{ route('admin.revistas.actualizar', $revista->id) }}" method="POST" enctype="multipart/form-data" class="space-y-5" id="form-revista">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <x-input.input type="text" id="titulo" name="titulo" value="{{ old('titulo', $revista->titulo) }}" required />
                    ...
                    <div>
                        ...
                        <x-input.input type="text" id="edicion" name="edicion" value="{{ old('edicion', $revista->edicion) }}" />
                ...
                <div>
                    ...
                    <x-input.input type="date" id="fecha_publicacion" name="fecha_publicacion" value="{{ old('fecha_publicacion', $revista->fecha_publicacion->format('Y-m-d')) }}" required />
                    @error('fecha_publicacion') <span class="text-red-500 text-xs mt-1">{{ $message }}</span> @enderror
                </div>

                <div>
                    <x-input.label for="archivo_pdf">Archivo PDF (Opcional)</x-input.label>
                    <input type="file" id="archivo_pdf" name="archivo_pdf" accept="application/pdf" class="block w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-md file:border-0 file:text-sm file:font-semibold file:bg-blue-50 file:text-blue-700 hover:file:bg-blue-100">
                    <p class="text-xs text-gray-500 mt-1">Solo sube un archivo si deseas reemplazar el actual.</p>
                    @error('archivo_pdf') <span class="text-red-500 text-xs mt-1">{{ $message }}</span> @enderror
                    
                    @if($revista->archivo_pdf)
                        <div class="mt-2 text-sm text-gray-600">
                            <strong>Archivo actual:</strong> <a href="{{ asset('storage/' . $revista->archivo_pdf) }}" target="_blank" class="text-blue-600 hover:underline"><i class="bx bxs-file-pdf"></i> Ver Archivo</a>
                        </div>
                    @endif
                </div>
    ...
    <!-- Script de extracción de portadas limpio -->
