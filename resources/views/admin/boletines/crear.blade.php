...
@section('titulo', 'Subir Boletín Informativo')

@section('content')
    <x-section class="space-y-4 p-4 sm:space-y-6 sm:p-6">
        <x-titulo titulo="Subir Boletín" />
        <x-breadcrumb>
        ...
        <x-card class="p-5 max-w-3xl mx-auto">
            <x-card.title class="mb-4 border-b pb-2"><i class="bx bx-upload"></i> Detalles del Boletín</x-card.title>

            <form action="{{ route('admin.boletines.guardar') }}" method="POST" enctype="multipart/form-data" class="space-y-5" id="form-boletin">
                @csrf
                ...
                <div>
                    ...
                    <x-input.input type="text" id="titulo" name="titulo" value="{{ old('titulo') }}" required placeholder="Ej. Boletín N° 15 - Logros de Mayo" />
                ...
                <div class="pt-4 border-t flex justify-end gap-2">
                    ...
                    <button type="submit" id="btn_guardar" class="inline-flex items-center gap-x-2 px-4 py-2 text-sm font-medium rounded-lg border border-transparent bg-blue-600 text-white hover:bg-blue-700 focus:outline-none focus:bg-blue-700">
                        <i class="bx bx-save mr-1"></i> <span id="btn_text">Guardar Boletín</span>
                    </button>
                </div>
            </form>
        </x-card>
