...
<script nonce="{{ app('csp-nonce') ?? '' }}">
    document.addEventListener('DOMContentLoaded', function() {
        ...
        const botones = document.querySelectorAll('.btn-ver-oficina');
        let modal;
        try {
            modal = new bootstrap.Modal(document.getElementById('modalOficinaInfo'));
        } catch (e) {
        ...
        botones.forEach(btn => {
            btn.addEventListener('click', function() {
                ...
                if (oficinasData[estado] && oficinasData[estado].length > 0) {
                    const accordionHtml = document.createElement('div');
                    
                    oficinasData[estado].forEach((oficina, index) => {
                        const itemHtml = `
                        <div class="border rounded-3 mb-3 shadow-sm bg-white overflow-hidden">
                            <div class="p-3 bg-light text-primary d-flex justify-content-between align-items-center custom-accordion-header" style="cursor: pointer;">
                                <span class="fw-bold"><i class="fas fa-building me-2"></i> ${oficina.nombre}</span>
                            ...
                            <div class="custom-accordion-body" style="background-color: #fff;">
                                <div class="d-flex align-items-start mb-2">
                                    ...
                                    <div>
                                        ...
                                        <span class="text-secondary">${oficina.direccion || oficina.facility || 'Información no disponible'}</span>
                                    </div>
                                </div>
                                <div class="d-flex align-items-start">
                                    ...
                                    <div>
                                        ...
                                        <span class="text-secondary">${oficina.telefono}</span>
                                    </div>
                                </div>
                            </div>
                        </div>
        ...
        const oficinasContainer = document.getElementById('oficinasListContainer');
        if (oficinasContainer) {
            oficinasContainer.addEventListener('click', function(e) {
