...
@section('titulo', 'Localización Farmacias IVSS - IVSS')
...
<div class="position-relative overflow-hidden text-center bg-light" style="padding: 100px 0 60px 0;">
    ...
    <div class="position-absolute top-0 start-0 w-100 h-100" style="background-image: url('{{ asset('img/medical-aesthetic-1.png') }}'); background-size: cover; background-position: center; opacity: 0.2; z-index: 0;"></div>
        ...
        <div class="row justify-content-center">
            <div class="col-lg-8" data-aos="fade-up">
                <div class="bg-white rounded-circle d-inline-flex align-items-center justify-content-center mb-4 shadow" style="width: 100px; height: 100px;">
                    <i class="fas fa-clinic-medical text-primary fa-3x"></i>
                </div>
                <h1 class="display-4 fw-bold text-white mb-3">Farmacias de Alto Costo IVSS</h1>
                <p class="lead text-white-50 mb-0">Seleccione en la siguiente lista para obtener información sobre las Farmacias IVSS ubicadas en su región.</p>
            </div>
        </div>
    </div>
        ...
        <div class="row justify-content-center mb-5" data-aos="fade-up">
            <div class="col-lg-6 text-center">
                <div class="input-group input-group-lg shadow-sm rounded-pill overflow-hidden">
                    ...
                    <input type="text" id="buscadorEstados" class="form-control border-0 px-3" placeholder="Buscar por estado (Ej. Miranda, Zulia...)" style="box-shadow: none;">
                </div>
            </div>
        </div>
            ...
            <div class="col-lg-3 col-md-4 col-sm-6 estado-item">
                <div class="card h-100 border-0 shadow-sm rounded-4 hover-elevate transition-all overflow-hidden">
                    <div class="card-body text-center p-4">
                        ...
                        <button class="btn btn-outline-primary rounded-pill w-100 fw-bold btn-ver-farmacia" data-estado="{{ $estado }}">
                            Ver Farmacias
                        </button>
                    </div>
                </div>
...
<!-- Modal de Farmacias (Placeholder funcional) -->
<div class="modal fade" id="modalFarmaciaInfo" tabindex="-1" aria-hidden="true">
        ...
        <div class="modal-content border-0 shadow-lg" style="border-radius: 1.5rem; overflow: hidden;">
            <div class="p-4 text-center position-relative">
                ...
                <h4 class="fw-bold text-dark mb-2">
                    Farmacias - <span id="modalEstadoNombre"></span>
                </h4>
                <p class="small text-muted mb-4">
                    Listado de Farmacias de Alto Costo disponibles en este estado.
                </p>

                <div id="farmaciasListContainer" class="text-start mb-4">
...
<script nonce="{{ app('csp-nonce') ?? '' }}">
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // Modal triggers
        const botones = document.querySelectorAll('.btn-ver-farmacia');
        const modal = new bootstrap.Modal(document.getElementById('modalFarmaciaInfo'));
        
        botones.forEach(btn => {
            btn.addEventListener('click', function() {
                const estado = this.getAttribute('data-estado');
                document.getElementById('modalEstadoNombre').textContent = estado;
                
                const container = document.getElementById('farmaciasListContainer');
                container.innerHTML = ''; // Clear previous

                if (farmaciasData[estado] && farmaciasData[estado].length > 0) {
                    const accordionHtml = document.createElement('div');
                    accordionHtml.className = 'accordion accordion-flush';
                    accordionHtml.id = 'accordionFarmacias';

                    farmaciasData[estado].forEach((farmacia, index) => {
                        const itemHtml = `
                        <div class="border rounded-3 mb-3 shadow-sm bg-white overflow-hidden">
                            <div class="p-3 bg-light text-primary d-flex justify-content-between align-items-center custom-accordion-header" style="cursor: pointer;">
                                <span class="fw-bold"><i class="fas fa-hospital-alt me-2"></i> ${farmacia.nombre}</span>
                            ...
                            <div class="custom-accordion-body" style="background-color: #fff;">
                                <div class="d-flex align-items-start mb-2">
                                    ...
                                    <div>
                                        ...
                                        <span class="text-secondary">${farmacia.direccion}</span>
                                    </div>
                                </div>
                                <div class="d-flex align-items-start">
                                    ...
                                    <div>
                                        ...
                                        <span class="text-secondary">${farmacia.telefono}</span>
                                    </div>
                                </div>
                            </div>
                        </div>
        ...
        // Event delegation for dynamically added accordions (Bypasses CSP inline-handler blocking)
        const farmaciasContainer = document.getElementById('farmaciasListContainer');
        if (farmaciasContainer) {
            farmaciasContainer.addEventListener('click', function(e) {
